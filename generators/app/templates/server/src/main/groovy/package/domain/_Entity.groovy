...
<% if (databaseType == 'sql') { %>
    ...
    <%_ } _%>
    Long id<% } %><% if (databaseType == 'mongodb') { %>
    @Id
    String id<% } %><% if (databaseType == 'cassandra') { %>
    @PartitionKey
    UUID id<% } %>
...
<%- formatAsFieldJavadoc(relationships[idx].javadoc) %>
    ...
    <%_     } _%>
    Set<<%= otherEntityNameCapitalized %>> <%= relationshipFieldNamePlural %> = new HashSet<>()
    ...
    <%_ }_%>
    <%= otherEntityNameCapitalized %> <%= relationshipFieldName %>
